...
%td = [0.5; 0.2; 0.1];
q = [0; pi/4; 0];                                          %Vector q inicial propuesto.
K = [1 0 0; 0 1 0; 0 0 1];                               %matriz de ganancias K.

t = 0.1;                   %Paso para cada instante de tiempo.

N = 150;                   %Numero de iteraciones.
Q = zeros(N,3);            %En esta matriz guardaremos las posiciones resuntantes de cada articulacion.
Q_d = zeros(N,3);          %En esta matriz guardaremos las velocidades resuntantes de cada articulacion.

M = zeros(N,1);            %Vector donde se guardaran los indices de manipulabilidad para cada iteracion.

for i=1:N
    
    Tq_i = bot.fkine([q; 0]);             %Obtenemos la matriz de transformacion para el vector q actual.
    v = td - Tq_i(1:3,4);                 %Error entre la posicion final deseada y la posicion actual del efector final.
    
    J = jacobian(bot,[q; 0]);             %Construimos jacobiano.
    q_d = pinv(J(1:3,1:3))*(K*v);         %Obtenemos la cinematica diferencial para el actual.
    ...
    M(i) = abs(det(J(1:3,1:3)));     %Indice de manipulabilidad.
end
